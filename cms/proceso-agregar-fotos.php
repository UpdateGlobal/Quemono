...
if($id=='OFE'){
	$uploaddir = 'assets/img/ofertas/'.$uploadname;
...
// Subir Imagen productos
if($id=='IHP'){
	$uploaddir = 'assets/img/productos/hover/'.$uploadname;
	if(move_uploaded_file($uploadtempname, $uploaddir)){
		$mensaje = "El archivo subi&oacute; correctamente";
	}else{
		$mensaje = "El archivo no se subi&oacute;";  
...
<script>
	function Cerrar(){
		...
		if(tNavegador=="Microsoft Internet Explorer"){
			...
			if(valor=="IHP") {
				opener.window.fcms.h_imagen.value = "<?php echo basename($_FILES['imagen']['name']); ?>";
			}
			if(valor=="FT") {
		...
		if(tNavegador=="Netscape"){
			...
			if(valor=="IHP") {
				window.opener.fcms.h_imagen.value = "<?php echo basename($_FILES['imagen']['name']); ?>";
			}
			if(valor=="FT") {
